        ...
        .subscribe({
          next: (page) => {
            this.dataSource.data = page.content.map((item) =>
              this.$deliveryAttemptService.convertToFE(item)
            );
          ...
          error: (error) => {
            ...
            this.dialogHandler.alert(
              "Failed to search delivery attempt data. " + this.$errorUtil.retrieErrorMessage(error)
            );
          },
        })
    );
      ...
      this.registerSubscription(
        this.$deliveryAttemptService.insertBatchByExcelFile(file).subscribe({
          ...
          error: (error) => {
            ...
            this.dialogHandler.alert(
              "Failed to import delivery attempt data. " + this.$errorUtil.retrieErrorMessage(error)
            );
          },
        })
      );
      ...
      this.registerSubscription(
        this.$deliveryAttemptService.updateBatchByExcelFile(file).subscribe({
          ...
          error: (error) => {
            ...
            this.dialogHandler.alert(
              "Failed to update delivery attempt data. " + this.$errorUtil.retrieErrorMessage(error)
            );
          },
        })
      );
